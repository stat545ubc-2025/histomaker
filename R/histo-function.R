...
#' dataset into the function. \code{data.frame}
...
#' readible. Didn't want to do var_of_int since int can sometimes be used as integer \code{list}
...
#' variable of interest. Named for what it represents, binwidth, for clarity. \code{numeric}
...
#' \code{"dodgerblue3"}, resulting in a blue fill colour. Named since this colour will be the
#' one used within the ggplot2 \code{fill} call. \code{string}
...
#' default is \code{"black"}, resulting in the a black outline. Named since this colour
...
histo_function <- function(dataset, var_of_interest, binwidth,
 ...
 # test to make sure that data is a proper dataframe
  ...
  var_enquo <- rlang::enquo(var_of_interest) # this stores the variable information name before processing - works like curly's but couldn't get those to work
  var_name <- rlang::as_name(var_enquo) # this converts the variable of interest to a name variable for labelling
  ...
  histogram <- dataset %>%
    dplyr::filter(!is.na(!!var_enquo)) %>% # remove any NA in the variable of interest before plotting; !! helps to unlock the variable from earlier
    ggplot2::ggplot( ggplot2::aes(x = !!var_enquo)) + # define the x variable for the histogram; !! helps to unlock the variable from earlier
    ggplot2::geom_histogram(binwidth = binwidth, fill = fill_colour, colour = outline_colour,
                            alpha = 0.9, linewidth = 0.1, position = "dodge") + # add design choices for clearer representation of data
    ggplot2::xlab(var_name) + # clean up labels/titles
    ggplot2::ylab("Count") +
    ggplot2::theme_minimal() # add a clearer theme
